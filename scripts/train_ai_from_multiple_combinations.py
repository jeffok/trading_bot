...
logger = get_logger("train_ai_multi")


def train_ai_from_multiple_combinations(
    symbol: str,
    ...
    dry_run: bool = False,
    balance_labels: bool = True,  # 是否平衡正负样本
) -> int:
    """使用多种信号组合训练AI模型
    ...
    Args:
        ...
        balance_labels: 是否平衡正负样本（避免模型偏向某一类）
    ...
    try:
        ...
        for combination in combinations:
            ...
            for i in range(1, len(rows)):
                ...
                if check_combination(
                    ...
                    signals.append({"idx": i, "direction": "LONG", "ai_score": ai_score, "kline": current, "combination": combination})
                ...
                if check_combination(
                    ...
                    signals.append({"idx": i, "direction": "SHORT", "ai_score": ai_score, "kline": current, "combination": combination})
            ...
            for signal in signals:
                ...
                trade["combination"] = signal["combination"]
        ...
        logger.info(f"所有组合共生成 {len(all_trades)} 笔交易")
        ...
        for trade in all_trades:
            combo = trade.get("combination", "unknown")
            if combo not in combination_stats:
                combination_stats[combo] = {"total": 0, "profit": 0, "loss": 0}
            combination_stats[combo]["total"] += 1
            if trade.get("net_pnl", 0) > 0:
                combination_stats[combo]["profit"] += 1
            else:
                combination_stats[combo]["loss"] += 1
        
        logger.info("各组合统计：")
        for combo, stats in sorted(combination_stats.items()):
            combo_win_rate = (stats["profit"] / stats["total"]) * 100.0 if stats["total"] > 0 else 0.0
            logger.info(f"  {combo}: 总计={stats['total']}, 盈利={stats['profit']}, 亏损={stats['loss']}, 胜率={combo_win_rate:.1f}%")
        
        # 平衡样本（如果启用）
        if balance_labels and len(winning_trades) > 0 and len(losing_trades) > 0:
            # 如果正负样本不平衡，进行下采样
            min_samples = min(len(winning_trades), len(losing_trades))
            if len(winning_trades) > min_samples:
                import random
                random.seed(42)  # 固定随机种子以便复现
                winning_trades = random.sample(winning_trades, min_samples)
                logger.info(f"平衡样本：盈利样本从 {len(winning_trades) + (len(all_trades) - len(winning_trades) - len(losing_trades))} 减少到 {min_samples}")
            if len(losing_trades) > min_samples:
                import random
                random.seed(42)
                losing_trades = random.sample(losing_trades, min_samples)
                logger.info(f"平衡样本：亏损样本从 {len(losing_trades)} 减少到 {min_samples}")
            
            all_trades = winning_trades + losing_trades
            logger.info(f"平衡后总样本数: {len(all_trades)}")
        
        # 限制交易数量
        if max_trades and len(all_trades) > max_trades:
            import random
            random.seed(42)
            all_trades = random.sample(all_trades, max_trades)
            logger.info(f"限制为 {max_trades} 笔交易")
        ...
        # 测试模型预测分布
        if not dry_run and ai_model is not None:
            logger.info("测试模型预测分布...")
            test_samples = all_trades[:min(100, len(all_trades))]
            predictions = []
            for trade in test_samples:
                try:
                    kline = trade.get("kline_data")
                    if kline:
                        x, _ = _vectorize_for_ai(kline)
                        proba_result = ai_model.predict_proba(x)
                        if isinstance(proba_result, list):
                            prob = float(proba_result[1])
                        else:
                            prob = float(proba_result)
                        predictions.append(prob * 100.0)
                except Exception:
                    pass
            ...
            if predictions:
                ...
                logger.info(f"预测评分分布: 平均={avg_score:.1f}, 最小={min_score:.1f}, 最大={max_score:.1f}")
                if avg_score > 90 or avg_score < 10:
                    logger.warning("⚠️  模型预测可能存在问题：评分过于极端")
        ...
        try:
            trace_id = f"train_ai_multi_{int(time.time() * 1000)}"
            ...
            try:
                save_current_model_blob(
                    ...
                    version='trained_from_multiple_combinations',
                    model_dict=ai_model.to_dict(),
                    metrics={
                        'seen': int(final_seen),
                        'trained_from': 'multiple_combinations',
            ...
            try:
                write_system_config(
                    db,
                    actor="train_ai_multi_script",
                    key=settings.ai_model_key,
                    value=json.dumps(ai_model.to_dict(), ensure_ascii=False),
                    trace_id=trace_id,
                    reason_code=ReasonCode.AI_TRAIN.value,
                    reason=f"AI model trained from {trained_count} trades across {len(combinations)} combinations, seen={final_seen}",
            ...
            logger.info(f"现在AI模型已经学习了多种信号组合的效果，可以给出更综合的评分")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="使用多种信号组合训练AI模型")
    parser.add_argument("--symbol", type=str, required=True, help="交易对符号（必需）")
    parser.add_argument("--combinations", type=str, nargs="+", required=True, help="要测试的组合列表，用+连接（必需）")
    ...
    parser.add_argument("--min-trades", type=int, default=50, dest="min_trades", help="最少需要多少笔交易才进行训练（默认：50）")
    ...
    parser.add_argument("--no-balance", action="store_true", dest="no_balance", help="不平衡正负样本（默认会平衡）")
    
    args = parser.parse_args()
    
    sys.exit(train_ai_from_multiple_combinations(
        symbol=args.symbol,
